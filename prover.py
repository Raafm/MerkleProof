...
def generate_upper_level_hashes(cur_level_hashes):
    next_level_hashes = []
    for i in range(0,len(cur_level_hashes),2):
        l = cur_level_hashes[i]
        r = cur_level_hashes[i+1]
        h = hash_internal_node(l,r)

        next_level_hashes.append(h)
...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        state = generate_upper_level_hashes(state)
        level_pos //= 2
